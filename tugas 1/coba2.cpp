...
void jumlahanData(node *head); // Perbaikan: Mengganti "jumlahData" menjadi "jumlahanData"
...
int main()
{
    ...
    do{
        ...
        fflush(stdin);
        scanf(" %c", &pilih); // Perbaikan: Menambahkan spasi sebelum %c
        if (pilih == '1')
            tambahAwal(&head);
        else if (pilih == '2')
            tambahData(&head);
            ...
            jumlahanData(head); // Perbaikan: Memanggil fungsi "jumlahanData" 
    } while (pilih != 'q');
